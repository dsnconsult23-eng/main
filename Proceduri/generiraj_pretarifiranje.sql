...
if ntpar_periodid in (621,623)  then  --IF1 begin
...
if tponuda_podbroj='000' then  --IF2 begin
...
if tprodukt='UL'   then  --IF3 begin
    if tpar_premijaid=0 then  --IF4 begin
...
if tdatum_pretarifiranje>= tdatum_vaznost then  --IF7 begin
...
if tpar_periodid<>0  and tpar_premijaid>=0  then  --IF8 begin
    ...
    if tpar_premijaid=0 then  --IF9 begin
        select nvl(iznos_premija,0)
        into tpremija
        from os_ponuda_detail
        where os_ponudaid=tos_ponudaid
    ...
    if tpolisa_broj1 like '28/%'  then  --IF10 begin
        execute  procedure ul_pretarifiranje_period (tos_polisaid ,tpar_premijaid ,tpremija ,tpar_periodid,tperiod , tuser_id ,tdatum_pretarifiranje,tdatum_vaznost )
    ...
    if terr1<>-1 then  --IF11 begin
        let tos_polisaid_nova=terr1;
    end if;  --IF11 end


    if terr1=-1 then  --IF12 begin
        ROLLBACK WORK;
        return -1,tporaka;
    end if;  --IF12 end
end if;  --IF8 end
...
if tpar_premijaid<>0  and tpar_periodid=0 then  --IF13 begin


    if nvl(tpremija,0) =0 then  --IF14 begin
        ROLLBACK WORK;
    ...
    where os_ponudaid=tos_ponudaid;
    if kolku_isp>0 then  --IF21 begin
    ...
    if (tos_produktid=60) or  (tos_produktid=622)  then  --IF29 begin
        let alfa2=40;
    end if;  --IF29 end

    if tos_produktid=141 then  --IF30 begin
        let alfa2=35;
    end if;  --IF30 end
    ...
    if tpar_trosoci_produkt ='002' then ----tuka po staro ---promil za OS 03.06.2014Slagjana  --IF34 begin
        ...
        if tosigurenik_par_client is null then  --IF35 begin
        ...
        while t<=tperiod_osig
            ...
            and kamatna_stapka=tkamatna_stapka
            and par_Tablica_SmrtId=tpar_Tablica_SmrtId;
            if t<k then  --IF36 begin
                let a_x_k=a_age_k;
            else  --IF36 else
                let a_x_k=0;
            end if;  --IF36 end
            if a_x_k=0 then  --IF37 begin
                let a_xt_kt=0 ;
            else  --IF37 else
                let a_xt_kt=(nx_tageplust- nx_agen)/dx_tageplust;
            end if;  --IF37 end
            if t<tperiod_osig then  --IF38 begin
                let a_x_n = (nx_age- nx_agen)/dx_age;
            else  --IF38 else
                let a_x_n=0;
            end if;  --IF38 end
            if t=tperiod_osig then  --IF39 begin
            ...
            if a_x_n=0 then  --IF40 begin
                let gp_doz=0;
                let gp_mesano=0;
            else  --IF40 else

                if   a_x_k=0 then  --IF41 begin
                    let gp_doz=0;
                    let gp_mesano=0;
                else  --IF41 else
            ...
            if  gp_doz = 0 then  let mr_doz_predpresm=0; else  --IF42 begin
                ...
                if gp_mesano=0 then  --IF43 begin
                    let pred_presm_mr =0;
                else  --IF43 else
        ...
        if tpar_trosoci_produkt ='003' then ----tuka po novo ---% za GP 03.06.2014 Slagjana  --IF48 begin
            ...
            ---**
            IF tpar_nacin_plati<>'005' then  --IF49 begin
            ...
            if tprodukt='ФУ'  or tprodukt='ФЗ' then  --IF51 begin


                if k=1 then--20.09.2017 promena  --IF52 begin
            ...
            if tprodukt='КЖ' then  --IF53 begin
            ...
            while t<=tperiod_osig
                ...
                if t<tperiod_osig then  --IF62 begin
                    let a_x_n = (nx_age- nx_agen)/ddx_age;
                else  --IF62 else
                    let a_x_n=0;
                end if;  --IF62 end

                if t>tperiod_osig then  --IF63 begin
                    let tneto_rezerva=0;
                    ...
                    if  tprodukt='КЖ' then  --IF65 begin
                    ...
                    if tzilimer_rezerva<0 then  --IF66 begin
                        let tzilimer_rezerva=0;
                        let trx_t=0;
                    ...
                    if tprodukt='ФУ'  or  tprodukt='ФЗ' then  --IF67 begin
                        let ageb=18;
                        let nb=7;
                        let a_xt_nt1=ddx_agen/ddx_tageplust;
                        let a_xt_kt=a_xt_kt;
                        if k=1 then  --IF68 begin
                        ...
                        if tzilimer_rezerva<0 then  --IF69 begin
                            let tzilimer_rezerva=0;
                        end if;  --IF69 end
                        ...
                        if mr_mesano<0 then  --IF70 begin
                            let mr_mesano=0;
                        end if;  --IF70 end
                        ...
                        if t=tperiod_osig then  --IF71 begin
                            let tkazna_otkup=0;
                        end if;  --IF71 end

                        let _otkup=mr_mesano*(1-tkazna_otkup/100);
                        if t<=godina_otkup then  --IF72 begin
                            let cv=0;
                        else  --IF72 else
                            if t>=k then  --IF73 begin
                                let pred_presm_mr=1;
                            else  --IF73 else
                            ...
                            if k=1   then  --IF74 begin
                                let cv=SA;
                            end if;  --IF74 end
                        end if;  --IF72 end

                        if tpol='F' then  --IF75 begin
                            let tpol_brak='M';
                        else  --IF75 else
                            let tpol_brak='M';
                        end if;  --IF75 end
                        ...
                        let taxt_kt_brak=0;
                        if k=1  then  --IF78 begin
                            let mr_mesano=tzilimer_rezerva;
                        else  --IF78 else
                            let mr_mesano=SA*(1-d/100*a_xt_nt+tcomision)+gp*(gamma1*a_xt_nt-a_xt_kt);
                        end if;  --IF78 end
                        if tprodukt='КЖ' then  --IF79 begin
                            let mr_mesano=0.04*a_xt_nt*(ddx_age/(nx_age-nx_agen));
                        end if;  --IF79 end
                        if mr_mesano<0 then  --IF80 begin
                            let mr_mesano=0;
                        end if;  --IF80 end
                        if tprodukt='КЖ' then  --IF81 begin
                            let tkazna_otkup=0;
                            if tneto_rezerva-tfaktor_zilimier<0 then  --IF82 begin
                                let _otkup=0;
                            else  --IF82 else
                                if t<=godina_otkup then  --IF83 begin
                                    let _otkup=0;
                                else  --IF83 else
                                    let _otkup=(tneto_rezerva-mr_mesano)*SA;
                                end if;  --IF83 end
                            end if;  --IF82 end
                            if t<=godina_otkup then  --IF84 begin
                                let cv=0;
                            else  --IF84 else
                            ...
                            if t=tperiod_osig then  --IF85 begin
                                let tkazna_otkup=0;
                            end if;  --IF85 end

                            let _otkup=mr_mesano*(1-tkazna_otkup/100);
                            if t<=godina_otkup then  --IF86 begin
                                let cv=0;
                            else  --IF86 else
                                if t>=k then  --IF87 begin
                                    let pred_presm_mr=1;
                                else  --IF87 else
        ...
        if tpar_trosoci_produkt ='002' then ----tuka po staro ---promil za OS 03.06.2014Slagjana  --IF90 begin

            ---sheet SA-----od kalkulatorot

            let platena_premija=tkap_premija*k;
            ---- sega za sega ke go ostavam fiksno otkako ke se resi  so provizijata sifrarnik
            if tosigurenik_par_client is null then  --IF91 begin
                let comision=(platena_premija*1.5/100)+(platena_premija*1/100)+(platena_premija*0.5/100)+(platena_premija*0.5/100);
            else  --IF91 else
                let comision=(platena_premija*3.30/100)+(platena_premija*1.60/100)+(platena_premija*1/100)+(platena_premija*0.60/100);
            end if;  --IF91 end
            ...
            while t<=tperiod_osig
                ...
                and kamatna_stapka=tkamatna_stapka
                and par_Tablica_SmrtId=tpar_Tablica_SmrtId;
                if t<k then  --IF92 begin
                    let a_x_k=a_age_k;
                else  --IF92 else
                    let a_x_k=0;
                end if;  --IF92 end
                if a_x_k=0 then  --IF93 begin
                    let a_xt_kt=0 ;
                else  --IF93 else
                    let a_xt_kt=(nx_tageplust- nx_agen)/dx_tageplust;
                end if;  --IF93 end
                if t<tperiod_osig then  --IF94 begin
                    let a_x_n = (nx_age- nx_agen)/dx_age;
                else  --IF94 else
                    let a_x_n=0;
                end if;  --IF94 end
                if t=tperiod_osig then  --IF95 begin
                    let cash_value_doz=0;
                    let cash_value_mesano=0;
                else  --IF95 else
                    let cash_value_doz=round(((dx_agen/dx_age)*SA)+(alfa/1000*SA)+(gama/1000*SA*a_x_n));
                ...
                if a_x_n=0 then  --IF96 begin
                    let gp_doz=0;
                    let gp_mesano=0;
                else  --IF96 else

                    if   a_x_k=0 then  --IF97 begin
                        let gp_doz=0;
                        let gp_mesano=0;
                    else  --IF97 else
                        let gp_doz=round((cash_value_doz/(a_x_k*(1-beta/100)))+(unit_cost/(1-beta/100)));
                ...
                if  gp_doz = 0 then  let mr_doz_predpresm=0; else  --IF98 begin
                    let mr_doz_predpresm=((((dx_agen/dx_age)*SA)+(alfa2/1000*SA))/a_x_k)*a_xt_kt;  end if;
                    --let mr_doz=((dx_agen/dx_tageplust*SA)- (dx_agen/dx_age*SA)+(alfa2/1000*SA))/a_x_k;
                    let mr_doz=round((dx_agen/dx_tageplust*SA)- mr_doz_predpresm);
                    if gp_mesano=0 then  --IF99 begin
                        let pred_presm_mr =0;
                    else  --IF99 else
                    ...
                    --      let mr_mesano=round((((mx_age -mx_agen)/dx_age+dx_agen/dx_age)*SA)- pred_presm_mr);
                    --      let mr_mesano=round((((mx_age -mx_agen)/dx_tageplust+dx_agen/dx_tageplust)*SA)- pred_presm_mr);
                    if mr_mesano<0 then  --IF100 begin
                        let CV=0;
                    else  --IF100 else
                        --let cv=mr_mesano*power((1+tkamatna_stapka/100),(tperiod_osig-t));
                        let cv=tkapitalizirana_vred;
                        if t=0 then  --IF101 begin
                            let net_profit=((((mx_age -mx_agen)/(dx_age+dx_agen))/dx_age)*SA)/a_xt_kt;
                        end if;  --IF101 end
                    end if;  --IF100 end
                    if t<tkoja_godina_pretarf then  --IF102 begin
                        let _otkup=0;
                    else  --IF102 else
                        let _otkup=mr_mesano;
                    end if;  --IF102 end
            ...
            if tpar_trosoci_produkt ='003' then  --IF104 begin
                ...
                IF tpar_nacin_plati<>'005' then  --IF105 begin
                    let db=comision/a_age_k; --alfa
                else  --IF105 else
                    let db=0.065; --alfa
                    let gama=1;
                ...
                if tprodukt='ФУ'  or tprodukt='ФЗ' then  --IF107 begin


                    if k=1 then--20.09.2017 promena  --IF108 begin
                ...
                if tprodukt='КЖ' then  --IF109 begin
                ...
                while t<=tperiod_osig
                    ...
                    let tcomision=0;
                    if k<>1 then  --IF110 begin
                        if t=0 then  --IF111 begin
                            let tcomision= 3.30/100+(1.60/100*l2/l1*discont)+(1/100*l3/l1*power(discont,2))+(0.60/100*l4/l1*power(discont,3));

                        end if;  --IF111 end
                        if t=1 then  --IF112 begin
                            let tcomision= 1.60/100+(1/100*l3/l1*discont)+(0.60/100*l4/l1*power(discont,2));


                        end if;  --IF112 end
                        if t=2 then  --IF113 begin
                            let tcomision= 1/100+(0.60/100*l4/l1*discont);


                        end if;  --IF113 end
                        if t=3 then  --IF114 begin
                            let tcomision= 0.60/100;
                        end if;  --IF114 end
                    else  --IF110 else
                        if t=0 and ( tprodukt='ФУ'  or tprodukt='ФЗ') then  --IF115 begin
                            let tcomision=0.065;
                        end if;  --IF115 end
                    end if;  --IF110 end

                    if t<k then  --IF116 begin
                        let a_x_k=a_age_k;
                    else  --IF116 else
                        let a_x_k=0;
                    end if;  --IF116 end
                    ...
                    if t<tperiod_osig then  --IF118 begin
                        let a_x_n = (nx_age- nx_agen)/ddx_age;
                    else  --IF118 else
                        let a_x_n=0;
                    end if;  --IF118 end

                    if t>tperiod_osig then  --IF119 begin
                        let tneto_rezerva=0;
                        let tfaktor_zilimier=0;
                        let tzilimer_rezerva=0;
                        let mr_mesano=0;
                        ...
                        --let tzilimer_rezerva =tneto_rezerva-tfaktor_zilimier;
                        if              tprodukt='КЖ' then  --IF121 begin
                            let neto_premija=neto_premija;
                        ...
                        if tzilimer_rezerva<0 then  --IF122 begin
                            let tzilimer_rezerva=0;
                            let trx_t=0;
                        ...
                        if tprodukt='ФУ'  or                tprodukt='ФЗ' then  --IF123 begin
                            let ageb=18;
                            let nb=7;
                            let a_xt_nt1=ddx_agen/ddx_tageplust;
                            let a_xt_kt=a_xt_kt;
                            if k=1 then  --IF124 begin
                                let trx_t=(mx_agen-k*mx_agen)/ddx_tageplust;
                            else  --IF124 else
                                let trx_t=(rx_tageplust-rx_agek+t*mx_agen-k*mx_agen)/ddx_tageplust;
                            end if;  --IF124 end
                            ...
                            let tfaktor_zilimier=(3/100)*(a_xt_kt/a_age_k);
                            let tzilimer_rezerva =tneto_rezerva-tfaktor_zilimier*SA;
                            if tzilimer_rezerva<0 then  --IF125 begin
                                let tzilimer_rezerva=0;
                            end if;  --IF125 end
                            ...
                            if mr_mesano<0 then  --IF126 begin
                                let mr_mesano=0;
                            end if;  --IF126 end
                            ...
                            if t=tperiod_osig then  --IF127 begin
                                let tkazna_otkup=0;
                            end if;  --IF127 end

                            let _otkup=mr_mesano*(1-tkazna_otkup/100);
                            if t<=godina_otkup then  --IF128 begin
                                let cv=0;
                            else  --IF128 else
                                if t>=k then  --IF129 begin
                                    let pred_presm_mr=1;
                                else  --IF129 else
                                ...
                                let cv=mr_mesano*power((1+tkamatna_stapka/100),(tperiod_osig-t))*pred_presm_mr;
                                if k=1   then  --IF130 begin
                                    let cv=SA;
                                end if;  --IF130 end
                            end if;  --IF128 end

                            if tpol='F' then  --IF131 begin
                                let tpol_brak='M';
                            else  --IF131 else
                                let tpol_brak='M';
                            end if;  --IF131 end
                            ...
                            let SAb=round(SA*tedin_osig_suma_brak,2);
                            let tneto_rezerva_brak=(mx_tagebplust-mx_tagenb+ddx_tagenb)/ddx_agebplust* SAb +0.003*taxt_kt_brak*SAb;
                            if t<7 then  --IF132 begin
                                let totkupna_vred_brak=0.92*tneto_rezerva_brak;
                            else  --IF132 else
                                if t=7 then  --IF133 begin
                                    let totkupna_vred_brak=tneto_rezerva_brak;
                                else  --IF133 else
                                    let totkupna_vred_brak=null;
                                end if;  --IF133 end
                            ...
                            if tprodukt='КЖ' then  --IF135 begin
                                let mr_mesano=0.04*a_xt_nt*(ddx_age/(nx_age-nx_agen));
                            end if;  --IF135 end
                            if mr_mesano<0 then  --IF136 begin
                                let mr_mesano=0;
                            end if;  --IF136 end
                            if tprodukt='КЖ' then  --IF137 begin
                                let tkazna_otkup=0;
                                if tneto_rezerva-tfaktor_zilimier<0 then  --IF138 begin
                                    let _otkup=0;
                                else  --IF138 else
                                    if t<=godina_otkup then  --IF139 begin
                                        let _otkup=0;
                                    else  --IF139 else
                                        let _otkup=(tneto_rezerva-mr_mesano)*SA;
                                    end if;  --IF139 end
                                end if;  --IF138 end
                                if t<=godina_otkup then  --IF140 begin
                                    let cv=0;
                                else  --IF140 else
                                    let cv=_otkup*power((1+tkamatna_stapka/100),(tperiod_osig-t));
                                end if;  --IF140 end
                                ...
                                if t=tperiod_osig then  --IF141 begin
                                    let tkazna_otkup=0;
                                end if;  --IF141 end

                                let _otkup=mr_mesano*(1-tkazna_otkup/100);
                                if t<=godina_otkup then  --IF142 begin
                                    let cv=0;
                                else  --IF142 else
                                    if t>=k then  --IF143 begin
                                        let pred_presm_mr=1;
                                    else  --IF143 else
            ...
            foreach
                ...
                if tgodini_osiguruvanje=tperiod_osig then  --IF146 begin
                    update os_ponuda_detail set osig_suma=tkapitalizirana_vred
                    where os_ponudaid=tos_polisaid_nova
                    and ts_vid_rizik='Or';

                    if kolku_isp >0 then  --IF147 begin
...
let odreden_datum_sk=date(tskadenca_datum_od+tkoja_godina_pretarf units  year);
if tdatum_pretarifiranje<= tdatum_vaznost then  --IF148 begin
...
if dali_ima_aneks >0 then  --IF149 begin
    if (tgodisnica_aneks>tdatum_vaznost and za_naredna_godina='1') or (za_naredna_godina='0') then  --IF150 begin
        ...
        where os_aneksid= tos_aneksid_posleden );
        if dali_proknizeno> 0 then  --IF151 begin
            ...
            foreach
                ...
                if dali_proknizeno_stavka>0 then  --IF152 begin
        ...
        delete from fin_stavka
        where os_aneks_fakturaid in (
            select os_aneks_fakturaid from os_aneks_faktura
            where os_aneksid in (select os_aneksid from os_aneks where os_polisaid=tos_polisaid1)
        );
        delete from os_aneks_faktura
        where os_aneksid in (select os_aneksid from os_aneks where os_polisaid=tos_polisaid1);
        delete from os_aneks
